...
(_a = document.getElementById("loginForm")) === null || _a === void 0 ? void 0 : _a.addEventListener("submit", function (event) {
    ...
    if (user) {
        ...
        var name_1 = user.email.split("@")[0];
        var box = document.getElementById("loginFormBox");
        if (box) {
            box.innerHTML = "<h2>Welcome ".concat(name_1, "</h2>");
        }
        startTest();
    }
    else {
        failedAttempts++;
        alert("Invalid credentials. Attempts left: ".concat(3 - failedAttempts));
    }
    if (failedAttempts >= 3) {
...
function startTest() {
    var numberOfQs = document.querySelector(".numberOfQs");
...
function generateProblems(count) {
    ...
    for (var i = 0; i < count; i++) {
        ...
        label.textContent = "".concat(a, " ").concat(op, " ").concat(b, " = ");
        var input = document.createElement("input");
        input.type = "number";
    ...
    submit.addEventListener("click", function () {
        ...
        result.textContent = "Score: ".concat(score, "/").concat(count);
